...
export default function ConnectorsPage() {
  ...
  async function handleSync(connector: "gdrive" | "gmail" | "meet" | "otter") {
    ...
    try {
      ...
      setResult({ files_synced: 0, entities_extracted: 0, articles_generated: 0, errors: [t('syncFailed')] });
    } finally {
      setSyncing(null);
    }
  }

  if (!status) return <div style={{ color: "var(--color-text-muted)" }}>{t('loading', { ns: 'common' })}</div>;
    ...
    if (!connected) {
      return (
        <button onClick={handleConnect} disabled={!status.google_configured}
          ...
          style={{ background: "var(--color-dark)", color: "var(--color-text-on-brand)" }}>{t('connect', { ns: 'common' })}</button>
      );
    }
    return (
      <button onClick={() => handleSync(connector as "gdrive" | "gmail" | "meet" | "otter")} disabled={syncing !== null}
        ...
        {syncing === connector ? t('syncing', { ns: 'common' }) : label}
      </button>
    );
  };
  ...
  const statusBadge = (connected: boolean) => (
    <span className="text-xs px-2 py-0.5 flex items-center gap-1.5"
      ...
      {connected ? t('connected', { ns: 'common' }) : t('notConnected', { ns: 'common' })}
    </span>
  );
    ...
    <div>
        ...
        {connectors.map(c => (
          <div key={c.key} className="p-4" style={{ background: "var(--bg-surface)", boxShadow: "var(--shadow-card)" }}>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg" style={{ color: "var(--color-text)" }}>{t(`${c.key}.name`)}</h2>
              {statusBadge(c.connected)}
            </div>
            <p className="text-sm mb-3" style={{ color: "var(--color-text-muted)" }}>{t(`${c.key}.desc`)}</p>
            {connectorBtn(c.connected, c.key, c.syncLabel)}
          </div>
        ))}
        <div className="p-4" style={{ background: "var(--bg-surface)", boxShadow: "var(--shadow-card)" }}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg" style={{ color: "var(--color-text)" }}>{t('otter.name')}</h2>
            {statusBadge(status.otter.connected)}
          </div>
          <p className="text-sm mb-3" style={{ color: "var(--color-text-muted)" }}>{t('otter.desc')}</p>
          {!status.otter.connected ? (
            <div className="flex gap-2">
              <input type="password" value={otterKey} onChange={(e) => setOtterKey(e.target.value)} placeholder={t('otter.apiKeyPlaceholder')}
                aria-label={t('otter.apiKeyPlaceholder')}
              ...
              <button onClick={handleSaveOtterKey} disabled={savingKey || !otterKey.trim()}
                ...
                {savingKey ? t('saving', { ns: 'common' }) : t('save', { ns: 'common' })}
              </button>
            </div>
          ) : (
            <button onClick={() => handleSync("otter")} disabled={syncing !== null}
              className="text-sm px-3 py-2 uppercase cursor-pointer disabled:opacity-50 disabled:cursor-default"
              style={{ background: "var(--bg-surface-warm)", color: "var(--color-text)" }}>
              {syncing === "otter" ? t('syncing', { ns: 'common' }) : t('syncNow', { ns: 'common' })}
            </button>
          )}
        </div>
      </div>
      {result && (
        <div className="grid grid-cols-3 gap-6 text-center">
          {[
            { label: t('stats.filesSynced'), value: result.files_synced },
            { label: t('stats.entities'), value: result.entities_extracted },
            { label: t('stats.articles'), value: result.articles_generated },
          ].map(stat => (
