  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: t('files'), value: status.total_files },
          { label: t('concepts'), value: status.total_entities },
          { label: t('connections'), value: status.total_connections },
          { label: t('topics'), value: status.topic_count },
        ].map(stat => (
      ...
      {provider && (
        <div className="mb-8 p-4" style={{ background: "var(--bg-surface-warm)" }}>
          <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>{t('llm')} </span>
      ...
      {status.top_topics.length > 0 && (
        <div className="mb-6">
          <h2 className="text-2xl mb-3" style={{ color: "var(--color-text)", lineHeight: 1.33 }}>{t('topTopics')}</h2>
      ...
      {insights.length > 0 && (
        <div className="mt-6">
          <h2 className="text-2xl mb-3" style={{ color: "var(--color-text)", lineHeight: 1.33 }}>{t('healthInsights')}</h2>
